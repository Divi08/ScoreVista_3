...
export const analyzeText = async (text: string): Promise<TextAnalysisResult> => {
  ...
  if (parsedGeminiFeedback.estimatedIELTSBand) {
    ...
    overallScore = Math.round((overallScore * 0.3) + (adjustedScore * 0.7));
...
const createTextBlocks = (originalText: string, correctedText: string): TextBlock[] => {
  ...
  // Split corrected text if available
  ...
  if (correctedText) {
    ...
    // try to match them as best as possible or use the full text
    if (correctedBlocks.length !== originalBlocks.length) {
      // Simple case: if there's only one corrected block but multiple originals
      if (correctedBlocks.length === 1 && originalBlocks.length > 1) {
        correctedBlocks = [correctedText]; // Use full text
      }
      // If we have more corrected blocks than original, truncate
      else if (correctedBlocks.length > originalBlocks.length) {
        correctedBlocks = correctedBlocks.slice(0, originalBlocks.length);
      }
      // If we have fewer corrected blocks than original, pad with empty strings
      else if (correctedBlocks.length < originalBlocks.length) {
        while (correctedBlocks.length < originalBlocks.length) {
...
const generateExplanation = (type: IssueType, original: string, suggestion: string): string => {
  switch (type) {
    case 'grammar':
      return `Consider "${suggestion}" for correct grammar.`;
    case 'spelling':
      return `"${original}" might be misspelled. Consider "${suggestion}".`;
    case 'punctuation':
      return `Check punctuation: "${suggestion}".`;
    case 'clarity':
      return `For better clarity: "${suggestion}".`;
    case 'style':
      return `Consider: "${suggestion}" for better style.`;
    case 'vocabulary':
      return `A more precise word choice: "${suggestion}".`;
    default:
